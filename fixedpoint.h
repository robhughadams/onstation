...
//Have we popped up a dialog warning about invalid fixed points?
	...
	public:
		CFixedPoint(LPCSTR szStationName);
		~CFixedPoint();
		...
		void SetComment(LPCSTR szComment);
		void SetStationName(LPCSTR szStationName) {m_szName=szStationName;}
		//Information
		inline float GetX() const {return m_fXpos;}
		inline float GetY() const {return m_fYpos;}
		inline float GetZ() const {return m_fZpos;}
		inline LPCSTR GetName() const {return m_szName;}
		inline LPCSTR GetComment() const {return m_szComment;}
};
