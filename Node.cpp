...
{
	TRACE("===================\nNode %s\n",GetName());
	int iCount=m_SurveyShotList.GetSize();
...
void CNode::CreateComplexJunctionGeometry(CJunctionGeometryBox * pGeometry,BOOL bClosedLoops)
{
	...
	int iNumShots=GetNumberOfShots(NST_ANYSHOT);
	...
	CSurveyShot * shot[50];
	for (int i=0;i<sizeof(shot)/sizeof(CSurveyShot *);i++)
	{
		shot[i]=NULL;
	}
...
int CNode::GetNumberOfShots(NODE_SHOTTYPE type)
{
	switch (type)
	{
	case NST_ANYSHOT:
...
CSurveyShot * CNode::GetShotByIndex(int iIndex,NODE_SHOTTYPE type)
{
	int iCount=m_SurveyShotList.GetSize();
	for (int j=0;j<iCount;j++)
	{
...
int CNode::GetNumberOfWallOnlyShots()
{
...
CSurveyShot * CNode::GetWallOnlyByIndex(int iIndex)
{
	int iCount=m_WallsOnlyShots.GetSize();
	for (int j=0;j<iCount;j++)
	{
		CSurveyShot *shot=m_WallsOnlyShots.GetAt(j);
		if (shot->GetLeg()->IsActive(pSettings_G->m_iColorSchemeIndex))
