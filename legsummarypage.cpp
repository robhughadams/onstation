...
BOOL CLegSummaryPage::OnKillActive()
{
	...
	GetDlgItemText(IDC_COMMENTS,m_Leg->m_szDescription,sizeof(m_Leg->m_szDescription)-1);
	char szBuffer[256];
	CPdate32 * pDate=(CPdate32 *)GetDlgItem(IDC_DATEEDIT1);
	if (pDate!=NULL)
	{
		COleDateTime date=pDate->GetValue();
	...
	GetDlgItemText(IDC_SYSTEM,m_Leg->m_szSystem,sizeof(m_Leg->m_szSystem)-1);
