...
//	{
		...
		while (iPosY<-rBoundry.Height())	//Bit of a safety
		{
		 	...
		 	else
		 	{
				...
				{
					iLength=sprintf(szText,"%.0f",fLength);
				}
				else
				{
					iLength=sprintf(szText,"%.0f",MetersToFeet(fLength));
				}
				CPoint ptMMText;
...
BOOL CRuler::OnToolTipText(UINT uId,NMHDR * pNMHDR,LRESULT *pResult)
{
	...
	if (!theApp.GetSettings()->IsMetric())
	{
		lstrcpy(Text->szText,"Ruler - Shows scale in ft/inch");
	}
	else
	{
		lstrcpy(Text->szText,"Ruler - Shows scale in m/cm");
	}
	*pResult=0;
	return TRUE;
}

int CRuler::OnToolHitTest(CPoint point,TOOLINFO *pTI) const
{
	RECT r;
	GetClientRect(&r);
