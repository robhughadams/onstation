...
class CNode
{
    ...
    private:	//DON'T EVER TOUCH THESE
    	char m_szNodeName[MAX_STATION_NAME_LENGTH];			//name of the station for this node
	...
	public:
	    inline LPCSTR GetName() const {return m_szNodeName;}
		CSurveyLeg * GetLeg();
		...
		int GetNumberOfShots(NODE_SHOTTYPE);
		CSurveyShot * GetShotByIndex(int iIndex,NODE_SHOTTYPE);
		int GetNumberOfWallOnlyShots();
		CSurveyShot * GetWallOnlyByIndex(int iIndex);
