...
{
	h->ReadTokenBegin("Folder");
	SetName(h->ReadTokenString("FolderName"));
	while (TRUE)
	{
		LPCSTR szData=h->GetTokenLookaheadString(TRUE);
		if (lstrcmpi(szData,"End")==0)
		{
			h->ReadTokenEnd("Folder");
			return;
		}
		LPCSTR szBeginType=h->ReadTokenString("Begin");
		if (lstrcmpi(szBeginType,"Folder")==0)
		{
			h->UndoLastRead();
		...
		{
			if (lstrcmpi(szBeginType,"Survey")==0)
			{
				h->UndoLastRead();
			...
			{
				h->UndoLastRead();
				h->ReadTokenBegin("SurveyLeg");	//cause an error to happen
			}
		}
	}
}
...
{
	h->WriteToken("Begin","Folder");
	h->WriteToken("FolderName",GetName());
	GotoTop();
    CFolder *data=GetCurrent();
    while (data!=NULL)
    {
	...
	h->WriteToken("End","Folder");
}

#ifdef _DEBUG
void CRealFolder::Dump()
{
	TRACE("Dumping folder %s\n",GetName());
	GotoTop();
	CFolder *data=GetCurrent();
	while (data!=NULL)
	{
...
void CRealFolder::Add(CFolder *pData,CFolder * pInsertAfter)
{
	...
	else
	{
		...
		{
			TRACE("Inserting item %s after %s\n",pData->GetName(),data->GetName());
...
int CRealFolder::GetSize()
{
    return m_folderList.GetCount();
}

CRealFolder * CRealFolder::FindSubFolderByName(LPCSTR szName)
{
    GotoTop();
    CFolder *data=GetCurrent();
    while (data!=NULL)
...
CSurveyLeg * CRealFolder::FindSurveyByName(LPCSTR szName)
{
    GotoTop();
    CFolder *data=GetCurrent();
    while (data!=NULL)
...
BOOL CRealFolder::IsColorSchemeDataSimilar(int iOne,int iTwo)
{
    ...
    while (data!=NULL)
    {
		...
		{
			TRACE("Color Scheme data is not similar\n");
			return FALSE;
		}
		data=GetNext();
    }
