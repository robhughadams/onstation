...
void CSurfaceEditPage::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SURFACEALTITUDEGRID, m_myGridControl);
	//}}AFX_DATA_MAP
}
...
BOOL CSurfaceEditPage::OnKillActive()
{
	...
	m_bDirty=m_myGridControl.IsModified();
...
BOOL CSurfaceEditPage::OnSetActive()
{
	...
	m_myGridControl.SetRows(m_SurfaceData->GetGridHeight()+1);
	m_myGridControl.SetCols(m_SurfaceData->GetGridWidth()+1);
	CString cs;
	for (int i=0;i<m_SurfaceData->GetGridHeight();i++)
	{
		m_myGridControl.SetRow(i+1);
