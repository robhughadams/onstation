...
void CSurfaceData::Write(CFileHelper * h)
{
	...
	h->WriteToken("Begin","SurfaceData");

	h->WriteToken("SurfaceSouthCorner",m_fBottom);
	h->WriteToken("SurfaceWestCorner",m_fLeft);
	h->WriteToken("SurfaceGridSquareSize",m_fSquareSize);
	h->WriteToken("SurfaceGridEntriesNS",m_iGridHeight);
	h->WriteToken("SurfaceGridEntriesEW",m_iGridWidth);

	h->WriteToken("Declination",m_fGridNorth);

	h->WriteToken("Begin","SurfaceHeights");
	for (int y=0;y<m_iGridHeight;y++)
	{
		h->WritePartialToken("SurfaceHeights");
		for (int x=0;x<m_iGridWidth;x++)
		{
	...
	h->WriteToken("End","SurfaceHeights");
	h->WriteToken("End","SurfaceData");
}

void CSurfaceData::WriteFixedPoints(CFileHelper *h)
{
	h->WriteToken("Begin","Constrained Stations");
	for (int i=0;i<GetFixedPointCount();i++)
	{
		CFixedPoint *T=GetFixedPoint(i);
		T->Write(h);
	}
	h->WriteToken("End","Constrained Stations");
}


void CSurfaceData::ReadSurface(CFileHelper * h)
{
	h->ReadTokenBegin("SurfaceData");

	m_fBottom=h->ReadTokenFloat("SurfaceSouthCorner");
	m_fLeft=h->ReadTokenFloat("SurfaceWestCorner");
	m_fSquareSize=h->ReadTokenFloat("SurfaceGridSquareSize");
	m_iGridHeight=h->ReadTokenInt("SurfaceGridEntriesNS");
	m_iGridWidth=h->ReadTokenInt("SurfaceGridEntriesEW");
	m_fGridNorth=h->ReadTokenFloat("Declination");
	...
	h->ReadTokenBegin("SurfaceHeights");
	for (int y=0;y<m_iGridHeight;y++)
	{
		h->ReadPartialToken("SurfaceHeights");
		for (int x=0;x<m_iGridWidth;x++)
		{
			SetAltitude(x,y,h->ReadAppendedFloat());
		}
 	}
	h->ReadTokenEnd("SurfaceHeights");
	h->ReadTokenEnd("SurfaceData");
}

void CSurfaceData::ReadFixedPoints(CFileHelper *h)
{
	h->ReadTokenBegin("Constrained Stations");
	LPCSTR szData=h->GetTokenLookaheadString(TRUE);
	while (lstrcmpi(szData,"StationName")==0)
	{
	...
	h->ReadTokenEnd("Constrained Stations");
}
...
CFixedPoint * CSurfaceData::GetFixedPoint(int i)
{	
...
int CSurfaceData::GetFixedPointCount()
{
...
BOOL CSurfaceData::IsConstrainedNode(LPCSTR szName,int iColorScheme)
{
