...
public:
	...
	float position[3];
	BOOL bJunction;
	char szName[MAX_STATION_NAME_LENGTH];
};
...
class CGWorldOpenGL:public CGWorld
{
	...
	public:
		...
		void AddLabel(CNode *node,CPosMatrix & Pos,LPCSTR szText,BOOL bJunction,COLORREF crColor);
		...
		void DrawText(CDC * dc,CViewMatrix * pViewPort,STATIONSHOW show,CRect rViewPort,float fLeft,float fRight,float fTop,float fBottom);
		void DrawTaggedPoints(CDC *dc,CViewMatrix * pViewPort,CRect rViewPort,float fLeft,float fRight,float fTop,float fBottom,BOOL bEliminateFixed);
		void DrawGDILineSegments(CDC * dc,CViewMatrix * pViewMatrix, CRect rViewPort,float fLeft, float fRight,float fTop,float fBottom,int iLineSize,BOOL bClosureErrors);
