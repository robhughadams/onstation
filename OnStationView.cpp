...
void COnStationView::GetPrinterInformation(CDC * dc)
{
//	TRACE("GetPrinterInformation\n");
	CViewMatrix view;
	view.XPan(-m_fXPan);
	view.YPan(-m_fYPan);
	view.ZPan(-m_fZPan);
...
//	TRACE("LOMETRIC limits without borders are %i,%i %i,%i\n",m_PrintedAreaRectangle.left,m_PrintedAreaRectangle.top,m_PrintedAreaRectangle.right,m_PrintedAreaRectangle.bottom);

//Convert page width and height into LOMETRIC coordinates
	m_nPageHeight=dc->GetDeviceCaps(VERTSIZE)*10;
	m_nPageWidth=dc->GetDeviceCaps(HORZSIZE)*10;
...
//	TRACE("Final limits are %i,%i %i,%i\n",m_PrintedAreaRectangle.left,m_PrintedAreaRectangle.top,m_PrintedAreaRectangle.right,m_PrintedAreaRectangle.bottom);
...
/*
	...
	*/
	...
	//Draw the scale bar
		...
		wsprintf(szBuffer,"%s",GetPreferedUnitString(0.0f));
		...
		wsprintf(szBuffer,"%s",GetPreferedUnitString(fIncrement*(bShort?5.0f:10.0f)));
		pDC->DrawText(szBuffer,-1,&rText,DT_CENTER|DT_NOCLIP);
		...
		sprintf(szBuffer,"%s",GetPreferedScaleString(m_fMetersPerCM));
		pDC->DrawText(szBuffer,-1,&rText,DT_CENTER|DT_NOCLIP);
...
//	pDCOriginal->FillSolidRect(10,rPrintedPage.Height()+10,rPrintedPage.Width()-20,-rPrintedPage.Height()-20,RGB(0,255,0));
	...
	*/
	...
	CFont * pOldFont=pDCOriginal->SelectObject(pFont);
	m_MyGWorld->DrawText(pDCOriginal,&view,pSettings_G->m_ShowStationNames[iColorScheme],rPrintedPage,fLeft,fRight,fTop,fBottom);
	pDCOriginal->SelectObject(pOldFont);
	delete pFont;
...
void COnStationView::GetPrintedPageBoundaries(CRect * pPage,int iPage,float *fLeft,float *fTop,float *fRight,float *fBottom)
{
	...
	*fBottom=*fBottom/100.0f*m_fMetersPerCM;
	...
	TRACE("GetPrintedPageBoundaries(): Printed page area is %i,%i to %i,%i\n",pPage->left,pPage->top,pPage->right,pPage->bottom);
	TRACE("GetPrintedPageBoundaries(): GL World Area is %i,%i to %i,%i\n",(int)*fLeft,(int)*fTop,(int)*fRight,(int)*fBottom);
}
...
int COnStationView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	{
		TRACE0("Failed to create compass\n");
	}
...
void COnStationView::RecalculateGeometry(BOOL bRegenerateGeometry)
{
	...
	{
//		TRACE("Recalculating Dirty data\n");
		bDataChanged=TRUE;
		try
		{
...
{
	AfxMessageBox("Warning: Perspective is not working in this version.  Please do not report this as a bug");
...
void COnStationView::OnViewSetexactview() 
{
	...
	CSetExactViewSheet SEVS(this,"Set View Point",m_fRotateClockwise,m_fRotateTilt,
...
void COnStationView::OnViewCloseloops() 
{
	...
	{
		AfxMessageBox("You must keep loop closure turned on in blunder color scheme since the loop closure code calculates the accuracy of shots.");
	}
	else
	{
...
//		dc.Rectangle(32,75,187,333);
		...
		r.top=32;
		r.bottom=17;
		dc.DrawText("Hello",-1,&r,DT_LEFT);	//internationalization OK
	}
	lTick=GetTickCount()-lTick;
	wsprintf(szResult,"In Bounds 1 pen 10000 reps %i",lTick);	//internationalization OK
	CPen pen2(PS_SOLID,1,RGB(0,0,0));
	MessageBox(szResult,"Time",MB_OK);	//internationalization OK
	...
	lTick=GetTickCount()-lTick;
	wsprintf(szResult,"Selecting 2 pens 10000 reps %i",lTick);	//internationaliztion OK
	MessageBox(szResult,"Time",MB_OK);	//internationalization OK
	...
	lTick=GetTickCount()-lTick;
	wsprintf(szResult,"multiply test 200000 reps %i",lTick);	//internationalization OK
	MessageBox(szResult,"Time",MB_OK);	//internationalization OK
...
//	{
		...
		of.lpstrFilter="(VRML Files)\0*.wrl\0*.wrld\0\0";
		...
		of.lpstrTitle="Export to VRML File";
...
//	catch
//	{
//		AfxMessageBox("Error during export.  Unable to complete operation.");
//	}
}
...
void COnStationView::RefreshStatusIndicators(CMainFrame *frm)
{
	...
	szBuffer.Format("%s",GetPreferedScaleString(m_fMetersPerCM));
...
void COnStationView::DrawGDIOverlay(CDC * pDC,int iHalfOfScreen)
{
	if (!m_bRotatingSystem || !pSettings_G->m_bRotateNoSolids[pSettings_G->m_iColorSchemeIndex])
	{
		...
		m_MyGWorld->DrawText(pDC,&view,pSettings_G->m_ShowStationNames[iColorScheme],r,fLeft,fRight,fTop,fBottom);
...
void COnStationView::OnTimer(UINT nIDEvent) 
{
...
void COnStationView::OnExportDxf()
{
	...
	of.hwndOwner=m_hWnd;
	of.hInstance=AfxGetInstanceHandle();
	of.lpstrFilter="(DXF Files)\0*.dxf\0\0";
	of.lpstrCustomFilter=NULL;
	of.nFilterIndex=0;
	of.lpstrFile=szReturn;
	of.nMaxFile=sizeof(szReturn);
	of.lpstrFileTitle=NULL;
	of.lpstrInitialDir=NULL;
	of.lpstrTitle="Export to DXF File";
	of.Flags=OFN_HIDEREADONLY;
	of.lpstrDefExt="dxf";
...
void COnStationView::ShowCompass(BOOL bShow)
{
	...
	else
	{
		if (bShow)
		{
			if (!m_Compass.Create(this,106))
			{
				TRACE0("Failed to create compass in showcompass\n");
			}
		}
	}
}
...
void COnStationView::OnQueryBlunders()
{
	if (pSettings_G->m_iColorSchemeIndex!=COLORSCHEME_BLUNDER)
	{
		if ((AfxMessageBox("Would you like your color scheme to automatically be set to blunder coloring.  This will color shots based on the likely amount of error."),MB_YESNO)==IDYES)
		{
