	...
	*pString="";
	...
	if (m_dwStartDate!=0xffffffff || m_dwEndDate!=0xffffffff)
	{
		...
		*pString+=csTemp;
		...
		if (m_dwStartDate!=0xffffffff)
		{
			csTemp.Format(" %i.%i.%i",GetYear(m_dwStartDate),GetMonth(m_dwStartDate),GetDay(m_dwStartDate));
			*pString+=csTemp;
		}
		if (m_dwEndDate!=0xffffffff)
		{
			csTemp.Format(" %i.%i.%i",GetYear(m_dwEndDate),GetMonth(m_dwEndDate),GetDay(m_dwEndDate));
			*pString+=csTemp;
		}
