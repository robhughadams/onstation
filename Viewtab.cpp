...
void CViewTab::DrawTab(CDC& dc,int iLeft,int iWidth,CRect& r,HBRUSH hMiddle,LPCSTR szText)
{
	POINT pts[4];
	pts[0].x=iLeft-5;
	pts[1].x=iLeft+3;
...
LONG CViewTab::OnHelpHitTest(UINT,LONG)
{
	return 0l;
}
...
BOOL CViewTab::OnToolTipText(UINT uId,NMHDR * pNMHDR,LRESULT *pResult)
{
	...
	lstrcpy(Text->szText,"Switches View Types");
	*pResult=0;
	return TRUE;
}
...
int CViewTab::OnToolHitTest(CPoint point,TOOLINFO *pTI) const
{
	RECT r;
	GetClientRect(&r);
