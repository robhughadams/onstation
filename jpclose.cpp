	...
	for (row=0 ; row<numeq ; row++) {
		...
		for (RowIndex = 0 ; RowIndex < numeq ; RowIndex++){
			...
			if(ColumnIndex <= ColumnIndexHigh){
				TRACE(" %10.4f", m_A[ColumnIndex]);
			}else{
				TRACE(" %10.4f", 0.0f);
			}
		}
		TRACE("  RHS value= %10.4f ",m_B[row]);
	}
...
// TRACE("     row= %li Tcol= RHS  Value = %f \n",TargetRow,m_B[TargetRow]);
		...
		// fix RHS
// TRACE("           row= %li Tcol= RHS  Value = %f  tmp= %f \n",row,m_B[row],tmp);
		m_B[row] *= tmp;
// TRACE("Normalised row= %li Tcol= RHS  Value = %f \n",row,m_B[row]);
	}
...
// TRACE("band 2nd pass row= %li \n",row);
//TRACE("     loop:     row= %li numeq = %li \n",row,numeq);
		...
		for (RowIndex = ColumnIndexLow ;
			...
			if(TargetRow < row){
				...
				if(ColumnIndex3 > ColumnIndexHigh3){
					TRACE("SolveBand: Severe Error: No target column found! row = %li col = %li \n"
						,TargetRow,TargetColumn);
					ASSERT(FALSE);
					return FALSE;
				}
...
// TRACE("     Trow= %li Tcol= %li  Value = %f \n",TargetRow,TargetColumn,m_A[ColumnIndex3]);
// TRACE("         m_B[row] Value = %f Targ=  %f \n",m_B[row],m_B[TargetRow]);
...
//TRACE("final     row= %li Tcol= RHS  Value = %f \n",row,m_B[row]);
		*(m_ANS[row]) = m_B[row];
...
/*		testit = AfxCheckMemory();
 ...
 TRACE("done loop:     row= %li numeq = %li \n",row,numeq);
